...
func AdminUsersList(c *gin.Context) {
    ...
    if err := gdb.Order("id asc").Find(&users).Error; err != nil { c.JSON(500, gin.H{"error":"db"}); return }
    c.JSON(200, users)
}
...
func AdminUsersCreate(c *gin.Context) {
    ...
    if err := c.ShouldBindJSON(&body); err != nil || body.Email == "" || body.Password == "" {
        c.JSON(400, gin.H{"error": "invalid_payload"}); return
    }
    ...
    if err := gdb.Create(&u).Error; err != nil { c.JSON(409, gin.H{"error":"conflict_or_db"}); return }
    c.JSON(201, u)
}
...
func AdminUsersUpdate(c *gin.Context) {
    ...
    var u models.User
    if err := gdb.First(&u, id).Error; err != nil { c.JSON(404, gin.H{"error":"not_found"}); return }
    var body struct{ Email *string `json:"email"`; Password *string `json:"password"`; Role *string `json:"role"` }
    if err := c.ShouldBindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"invalid_payload"}); return }
    if body.Email != nil { u.Email = *body.Email }
    if body.Role != nil { u.Role = *body.Role }
    if body.Password != nil && *body.Password != "" {
        if hashed, err := hashPassword(*body.Password); err == nil { u.Password = hashed } else { c.JSON(500, gin.H{"error":"hash"}); return }
    }
    if err := gdb.Save(&u).Error; err != nil { c.JSON(500, gin.H{"error":"db"}); return }
    c.JSON(200, u)
}
...
func AdminUsersDelete(c *gin.Context) {
    gdb := dbpkg.Get(); if gdb == nil { if _, err := dbpkg.Init(); err != nil { c.JSON(500, gin.H{"error":"db"}); return }; gdb = dbpkg.Get() }
    id, _ := strconv.Atoi(c.Param("userId"))
    if err := gdb.Delete(&models.User{}, id).Error; err != nil { c.JSON(500, gin.H{"error":"db"}); return }
