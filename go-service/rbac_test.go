...
func TestRBAC_ProjectRoles(t *testing.T) {
    ...
    var u2 models.User
    if err := gdb.Where("email = ?", "u2@test.local").First(&u2).Error; err != nil { t.Fatalf("u2 not found: %v", err) }
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("viewer get %d %s", w.Code, w.Body.String()) }
    ...
    req.Header.Set("Authorization", "Bearer "+tok1)
    r.ServeHTTP(w, req)
    if w.Code != http.StatusNoContent { t.Fatalf("owner delete %d %s", w.Code, w.Body.String()) }
}
